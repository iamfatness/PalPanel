...
public class ProcessSupervisorTests
{
    ...
    [Fact]
    public async Task Adopt_WhileRunning_IsNoOp()
    {
        var l = new FakeLauncher(); var s = Make(l);
        await s.StartAsync(CancellationToken.None);
        s.MarkRunning();
        var runningSince = s.RunningSince;
        l.Existing = new FakeProcess();
        s.AdoptExistingIfRunning();
        Assert.Equal(ServerState.Running, s.State);
        Assert.Equal(runningSince, s.RunningSince);
        Assert.Single(l.Launched); // no adoption swapped in the process
    }
